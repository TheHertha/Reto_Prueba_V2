<?php
require_once __DIR__ . '/config.php'; // usa el PDO global $pdo
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['accion']) && $_POST['accion'] === 'crear_producto') {
  ...
  if ($nombre !== '') {
    $sql = "INSERT INTO fis_productos (sku, nombre, precio_unitario, stock)
            VALUES (:sku,:nombre,:precio,:stock)
...
if ($busqueda !== '') {
  $like = "%$busqueda%";
  $st = $pdo->prepare("SELECT id, sku, nombre, precio_unitario, stock
                       FROM fis_productos
                       WHERE nombre LIKE :q OR sku LIKE :q
                       ORDER BY nombre ASC");
  $st->execute([':q' => $like]);
} else {
  $st = $pdo->query("SELECT id, sku, nombre, precio_unitario, stock
                     FROM fis_productos
                     ORDER BY nombre ASC");
}
...
$totales = $st->fetch(PDO::FETCH_ASSOC) ?: ['ingresos'=>0,'piezas'=>0];

// ----- Resumen últimos 7 días -----
$resumenDias = $pdo->query(
...
<style>
  :root{ --bg:#f5f5f5; --card:#fff; --ink:#333; --brand:#000; --accent:#FFD700; --muted:#666; --line:#ddd; --ok-bg:#e6f4ea; }
  *{box-sizing:border-box} body{margin:0;background:var(--bg);color:var(--ink);font-family:'Segoe UI',Tahoma,Arial,sans-serif}
  .layout{display:flex;min-height:100vh} .sidebar{width:250px;background:var(--brand);color:#fff;padding:20px;display:flex;flex-direction:column;gap:20px}
  .sidebar-header{display:flex;align-items:center;gap:12px;padding-bottom:16px;border-bottom:1px solid #333}
  .logo{width:46px;height:46px;border-radius:8px;object-fit:cover;transition:.3s;background:#111}.logo:hover{transform:rotate(5deg)}
  ...
  .nav-item:hover{background:var(--accent);color:#000}.nav-item.active{background:var(--accent);color:#000}
  .main{flex:1;padding:40px;background:var(--card)} .header{display:flex;gap:12px;align-items:center;justify-content:space-between;flex-wrap:wrap;margin-bottom:22px}
  ...
  .btn{cursor:pointer;position:relative;overflow:hidden}.btn.primary{background:var(--brand);color:var(--accent);border:1px solid var(--accent)}
  .btn.primary:hover{color:#000;background:var(--accent)} .btn.ghost{background:transparent;color:var(--brand);border:1px solid var(--line)}
  ...
  .kpi{background:var(--ok-bg)} .muted{color:var(--muted);font-size:.92rem}
  .album{display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:12px}
  .day{background:var(--card);border:1px solid var(--line);border-radius:10px;padding:12px}.day h4{margin:0 0 6px 0;font-size:1rem}.day .muted{font-size:.85rem}
  ...
  tr:hover{background:#fafafa}.danger{background:#ffe9e9}
  @media(max-width:1024px){.layout{flex-direction:column}.sidebar{width:100%}.main{padding:20px}.span-7,.span-5{grid-column:span 12}}
</style>
</head>
<body>
  <div class="layout">
    <aside class="sidebar">
      <div class="sidebar-header">
        <img src="assets/img/1-4.png" class="logo" alt="CAT21"><h2>CAT21 Admin</h2>
      </div>
    ...
    <main class="main">
      ...
      <section class="cards">
        <div class="card span-7">
          ...
          <p class="muted" style="margin-top:6px">Si el SKU existe, se actualizan nombre, precio y stock.</p>
        </div>
      ...
      <div class="album">
        <?php foreach($resumenDias as $d): ?>
          <div class="day">
            ...
            <div style="margin-top:8px"><a class="pill" href="?fecha=<?php echo htmlspecialchars($d['dia']); ?>">ver</a></div>
          </div>
        <?php endforeach; ?>
      </div>
      ...
      <table>
        <thead><tr><th>SKU</th><th>Nombre</th><th>Precio</th><th>Stock</th><th>Vender</th></tr></thead>
        <tbody>
...
(function(){
  ...
  refreshTotals();
  if(!fechaSel){ setInterval(refreshTotals, 5000); }
})();
</script>
</body>
</html>
